...
class ImportSeisWizard_Main_Window(qtw.QDialog):
    signal_seismic_name = qtc.pyqtSignal(str)

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

        self.ui_ImportSeisWiz = Ui_Seismic_Import_Dialog()
        self.ui_ImportSeisWiz.setupUi(self)

        self.is_2D_or_3D = 'No Geometry'
    ...
    def onClicked(self):
        ...
        if radioBtn.isChecked():
            self.is_2D_or_3D = radioBtn.text()
            print('Radio Button: ', self.is_2D_or_3D)

    # Method Import Seismic Button Data   
    def import_seis_window(self):
        if (self.is_2D_or_3D == '2D Seis'):
        ...
        elif (self.is_2D_or_3D == 'No Geometry'):
            self.seis_filename = qtw.QFileDialog.getOpenFileName(self, 'Open File','D:\\Documents\\SEGY_Importantes', 
            'Segy Files (*.sgy *.segy)')

            if (self.seis_filename != ('', '')):
                with segyio.open(self.seis_filename[0], 'r', ignore_geometry=True) as segyfileNoGeom:
                    header = segyio.tools.wrap(segyfileNoGeom.text[0])
                    self.ui_ImportSeisWiz.SegyHeader_textBrowser.setText(header)
                    self.ui_ImportSeisWiz.Seismic_FileName_lineEdit.setText(Path('{0}'.format(self.seis_filename[0])).stem)
